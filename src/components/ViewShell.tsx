...
interface ViewShellProps {
  ...
  subtitle?: string
  icon: LucideIcon
  logoSrc?: string // if set, shows the app logo instead of the icon square
 ...
 */
export function ViewShell({ title, subtitle, icon: Icon, logoSrc, action, children }: ViewShellProps) {
  return (
    <div className="flex h-full flex-col">
      <header className="safe-top flex items-center gap-3 border-b border-zinc-800 px-4 py-3">
        {logoSrc ? (
          <img src={logoSrc} alt="SquadSort" className="h-9 w-9 shrink-0 rounded-lg" />
        ) : (
